params <- list(outcomeName = c("COPDAE", "heart failure"),
               ...
               sigmaG = c(0.005, 0.001, 0.01),
               sigmaE = 0.005,
               DLag = c("DLag 0:13", "DLag 0:6"))

makeClusterDirs <- function(n, basedir = "cluster", offset = 0) {
    ...
    for(i in seq(along = dirNames)) {
        ...
        file.symlink("~/projects/multiDLM/approx-gibbs.R", dirNames[i])
...
writeParamsFile <- function(ptable, dirnames) {
    for(i in seq(along = dirnames)) {
        ...
        writeLines("source(\"approx-gibbs.R\")", con)
        ...
        writeLines("g <- gibbs(b, v, maxit = 25000, verbose = TRUE, covModel = covModel, sigmaE = sigmaE, sigmaG = sigmaG, do.plot = FALSE)", con)
