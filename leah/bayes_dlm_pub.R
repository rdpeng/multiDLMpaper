...
#####################################################
## a procFunc for buildDB, yielding basicNMMAPS plus lags 0:14 of
## pm10tmean and pm25tmean
#####################################################
...
pmNMMAPS <- function (dataframe) {
    ...
    varList <- c("date", "cvd", "death", "resp", "tmpd", "rmtmpd", "dptp", 
                 "rmdptp", "time", "agecat", "dow", "pm10tmean",
                 paste("l", 1:7, "pm10tmean", sep = ""), "pm25tmean", paste("l", 1:7, "pm25tmean", sep = ""))
...
bayesDLM <- function(citydata, pollutant = "pm10tmean", nLags = 14, sigma = 0.004,
    ...
    ## repeatedly update eta, theta, beta, save thetas, etas,
    ## reject/accept, and corr btw offsets
    ...
    for (i in 1:its) {
        eta.step <- updateEta(eta.matrix = eta.matrix, theta = theta,
                              inv.cov.hat = inv.cov.hat, nLags = nLags,
                              sigma = sigma)
        eta1 <- eta.step$eta1
        eta2 <- eta.step$eta2
        ...
        beta.step <- updateBeta(data = citydata, polloffset = polloffset,
                                oldbet = beta)
        beta <- beta.step$beta
        nonpolloffset <- beta.step$nonpolloffset
        ...
        est.matrix[i+1,] <- c(as.numeric(theta), eta1, eta2, theta.step$reject,
                              cor(nonpolloffset[notmiss], polloffset[notmiss]),
                              beta.step$reject)
    }
    ...
    names(rval) <- c(names(theta), "eta1", "eta2", "reject",
                     "offsetcor", "rejectbeta")
    invisible(rval)
}
...
updateBeta <- function(data, polloffset, oldbet = NULL) {
    ...
    fit <- fitPartialMod(data = cbind(data, polloffset = polloffset),
                         pollutant = NULL,
...
## Don't need this, there is already cov2cor in R [RDP]
##
## covToCor <- function(M) {
...
bayesDLMapprox <- function(data, pollutant = "pm10tmean", nLags = 14,
                           cause = "death", df.Time = 7 * 14, df.time = 1 * 14,
                           df.Temp = 6, df.Dew = 3, sigma = 0.004,
                           eta.mat = NULL) {
    ...
    if (is.null(eta.mat)) {
        hyper.mat <- expand.grid(seq(-0.35, -0.05, length = 10),
    ...
    mod <- fitSingleCity(data, pollutant = pollutant, nLags = nLags, cause = cause,
                         df.Time = df.Time, df.time = df.time, df.Temp = df.Temp,
                         df.Dew = df.Dew)
